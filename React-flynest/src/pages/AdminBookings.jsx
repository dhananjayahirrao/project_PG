...
const AirlineBooking = () => {
  ...
  useEffect(() => {
    if (!outboundBookingId || (!flightData && !outboundFlight)) {
      toast({
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      ...
      // Confirm outbound booking
      try {
        await bookingAPI.updateStatus(outboundBookingId, 'Confirmed');
      } catch (err) {
        console.error('Failed to confirm outbound booking:', err);
        success = false;
      }

      // Confirm return booking (if round-trip)
      if (isRoundTrip && returnBookingId) {
        try {
          await bookingAPI.updateStatus(returnBookingId, 'Confirmed');
        } catch (err) {
          console.error('Failed to confirm return booking:', err);
          success = false;
        }
      }
      ...
      toast({
        ...
        description: "But confirmation failed. Contact support with Booking ID."
      });
      setTimeout(() => navigate('/payments'), 2000);
    } finally {
      setLoading(false);
  ...
  const handleExternalBooking = () => {
    ...
    // Optional: window.open(`https://airline.com/booking/${outboundBookingId}`, '_blank');
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="glass-effect border-white/10">
              ...
              <CardContent className="space-y-4">
                ...
                {/* Always show "Pay" button unless already confirmed */}
                <Button
                  onClick={() => {
